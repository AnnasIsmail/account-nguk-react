          ...
          >
            {'error' in item ? (
              <Typography color="error" variant="h6">
                {key.replace('e', 'Session ').replace('a', ' ACT ')}
              </Typography>
            ) : (
          ...
          <AccordionDetails>
            {
              ...
              'error' in item ? (
                ...
                <Typography>
                  <Typography>Rank : {item.final_rank_patched}</Typography>
                  <Typography>Total Matches : {item.number_of_games}</Typography>
                  <Typography>Wins Matches : {item.wins}</Typography>
                  <Typography>Lost Matches : {item.number_of_games - item.wins}</Typography>
                </Typography>
              ) : null
            }
          </AccordionDetails>
    ...
    >
      <Fade in={props.open}>
        <Box sx={style}>
          ...
          <Typography>
            Highest Rank: <b>{detailSkin.data?.highest_rank.patched_tier}</b> in{' '}
            <b>{detailSkin.data?.highest_rank.season.replace('e', 'Session ').replace('a', ' ACT ')}</b>
          </Typography>
          {renderedData}
        </Box>
      </Fade>
