...
export default function UserManagement() {
  ...
  const getData = () => {
    ...
    axiosConfig.post('/access', { token: cookies.token }).then((response) => {
  ...
  const deleteAccess = () => {
    ...
    axiosConfig.post('/access/delete', { _id: dataUserDelete._id, token: cookies.token }).then((response) => {
  ...
  return (
    <Page title="User Management" style={{ height: '100%', width: 'calc(100% - 20px)', margin: '0 10px' }}>
      ...
      <AlertDelete
        open={openDelete}
        actionDelete={deleteAccess}
        handleClose={() => setOpenDelete(false)}
        nama={dataUserDelete.nama}
        message={`Apakah kamu yakin ingin menghapus akses `}
      />
    </Page>
  );
}
